...
function maxWords(sentences: string [] ): number{
...
function minOperationsThreshold(nums: number[], k: number):number{
    let count = 0; 
    for(let num of nums){
        if(num < k){
            count++
...
function sumOfTheIndicies(nums: number[], k: number):number{
    let sum = 0;
    for(let i = 0; i < nums.length; i++){
        let num = nums[i] ?? 0
...
function truncateSentence(strs:string, k:number):string{
...
function truncateSentences(strs:string, k:number):string{
    let words = strs.split(" ")
...
function decompress(nums: number[]):number[]{
    let result: number[] = [];
...
function removeOuterParenthesis(str: string):string{
    let result: string  = ""
    let count = 0;
    for(let i = 0; i < str.length; i++){
        let ch = str.charAt(i)
...
function countGoodTrplets(arr:number[], a:number, b:number, c:number ):number{
    let count = 0;
...
function stableMountains(heights:number[], threshold: number){
    let final:number[] = [];
...
function arrayStringsEqual(word1:string[], word2:string[]):boolean{
...
function sumOfSubarray(nums:number[], start:number, end:number):number{
    let sum = 0; 
...
function sumOfVariable(nums: number[]):number{
    let sum = 0;
    for(let i = 0; i< nums.length; i++){
        let numI = nums[i] ?? 0
        let end = i+1
...
function goodNumbers(num1:number[], num2: number[], k:number):number{
    let count = 0; 
 ...
 function decodeTheString(key: string, message: string):string{
...
function minimumSum4Digits(num: number): number {
  let str = "" + num;
  let arr = str.split("");
  arr.sort();
  console.log(arr)
