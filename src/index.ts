 function getSneakyNumber(nums:number[]) : number []{
    let map = new Map()
    for(let i: number = 0; i < nums.length; i++){
        let count: number = map.get(nums[i]) ?? 0
        count++
...
function kidsWithCandies(nums: number[], extracandies: number) : boolean [] {
...
function sumofString(str:string) : number {
    let sum:number = 0
...
function defangingIpaddress(address:string):string{
    let result: string = ""
...
function defangle(address:string):string{
    let res: string = ""
...
function defangleOne(address: string): string {
...
function interpret02(command: string): string {
    return command.replaceAll("()", "o").replaceAll("(al)", "al");
};

function numJewelsInStone(jewels: string, stones: string): number{
    let count: number = 0; 
    for(let ch  of stones ){
        let check:boolean = jewels.includes(ch)
        if(check){
            count++
        }
    }
...
function IsConsistent(allowed: string, words: string): boolean {
    for (let ch of words) {
...
function countConsistent(allowed: string, words:string[] ): number{
    let count: number = 0; 
...
function convertDateBinary(date:string):string{
    let result: string [] = [] 
    let dates: string[] = date.split("-")
    for(date of dates ){
...
function largestlocal(data: number[][]):number[][]{
    let m = data.length;
...
function reverseDegree(str:string):number{
...
function smallerNumber(nums:number[]): number[] {
    let res: number[] = []
...
function getState(nums: number[], k: number, mul: number): number[] {
    for(let i = 0; i < k; i++){
